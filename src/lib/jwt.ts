...
const verifyJwt = async (xsrfTokenReq: string, authHeader: string, userIsPresent = false):  Promise<CleanUser | void> => {
...
const base = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
	try {
...
export class jwtMiddleware {
	...
	static async acceptUser(req: Request, res: Response, next: NextFunction): Promise<void> {
		...
		if (
			...
			return next();
		error(req, res, 'JW_002', { status: 401 });
